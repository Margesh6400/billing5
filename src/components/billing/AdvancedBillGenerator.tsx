...
import { 
  AdvancedBillingCalculator, 
  PlateRates, 
  BillCalculation, 
  ExtraCharge, 
  Discount 
} from '../../utils/advancedBillingCalculator';
import { 
  generateAdvancedBillJPG, 
  downloadAdvancedBillJPG, 
  AdvancedBillData 
} from '../../utils/advancedBillJPGGenerator';
import { 
  Calculator, 
  Plus, 
  Trash2, 
...
interface AdvancedBillGeneratorProps {
  client: Client;
  onBillGenerated: () => void;
  onCancel: () => void;
}

export function AdvancedBillGenerator({ client, onBillGenerated, onCancel }: AdvancedBillGeneratorProps) {
  const { user } = useAuth();
  ...
  const calculator = new AdvancedBillingCalculator(plateRates, defaultRate);
  ...
  const handleGenerateBill = async () => {
    ...
    try {
      ...
      const advancedBillData: AdvancedBillData = {
        bill_number: billNumber,
        client: {
          id: client.id,
          name: client.name,
        ...
        total_challans: billCalculation.total_challans
      };

      // Generate and download JPG
      const jpgDataUrl = await generateAdvancedBillJPG(advancedBillData);
      const filename = `bill-${client.id}-${billDate}`;
      downloadAdvancedBillJPG(jpgDataUrl, filename);
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-gray-900">{billCalculation.total_challans}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
      ...
      <div className="flex items-center justify-between bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <div>
          <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            ...
            Advanced Bill Generator for {client.name}
          </h2>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            ...
            {showRateSettings ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
